...
drawerTabs.forEach(btn => btn.addEventListener('click', ()=> openTab(btn.dataset.tab)));
filterTabBtn.addEventListener('click', ()=> openTab('filter'));
toneTabBtn.addEventListener('click', ()=> openTab('tone'));
galleryTabBtn.addEventListener('click', ()=> openTab('gallery'));
...
async function startRecording(){
  ...
  try{
    ...
    state.recorder.onstop = () => {
      ...
      downloadDataUrl(url, `pixel-lomolab-rec-${stamp}.webm`, true);
...
function downloadDataUrl(url, name, rawUrl=false){
  ...
  return new Promise((resolve, reject)=>{
    ...
    req.onupgradeneeded = (event)=>{
      ...
      if(!db.objectStoreNames.contains('shots')){
        const store = db.createObjectStore('shots', { keyPath:'id', autoIncrement:true });
